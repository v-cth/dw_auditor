...
def validate_audit_results(results: Any, require_columns: bool = True) -> AuditResultsDict:
    """
    Validate that results dictionary has the expected structure

    Args:
        results: Results dictionary to validate
        require_columns: Whether 'columns' field is required (default: True)
    ...
    required_keys = ['table_name', 'total_rows', 'analyzed_rows', 'sampled']
    if require_columns:
        required_keys.append('columns')
